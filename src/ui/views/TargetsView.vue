...
async function saveTarget(): Promise<void> {
  if (!targetFormData.name || !targetFormData.url) {
    props.addToast('名称和 URL 是必填项', 'warning')
    return
  }
  ...
  try {
    ...
    props.addToast(data.message || '上传目标添加成功', 'success')
...
function deleteTarget(name: string, global: boolean): void {
  props.openConfirm('确认删除', `确定要删除上传目标 ${name} 吗？`, 'danger', async () => {
    try {
      const data = await api.deleteTarget({ name, global })
      props.addToast(data.message || '删除成功', 'success')
      await fetchTargets()
    }
    catch (err) {
      props.addToast(String(err), 'error')
...
function deleteTargetSkill(targetName: string, skillName: string, global: boolean): void {
  props.openConfirm('确认删除', `确定要从远端库 ${targetName} 中删除 Skill: ${skillName} 吗？`, 'danger', async () => {
    try {
      await api.deleteTargetSkill({ targetName, skillName, global })
      props.addToast('删除成功', 'success')
...
<template>
  <div class="animate-fade-in w-full space-y-12 pb-10">
    <section>
      ...
      <div v-if="targets.length === 0" class="glass flex flex-col items-center rounded-2xl border-2 border-dashed border-slate-800 p-16 text-center text-slate-400">
      ...
      <div v-else class="space-y-6">
        <div
          ...
          :key="target.name"
          class="glass group relative flex flex-col overflow-hidden rounded-2xl border border-slate-700/50 p-6 transition-all duration-300 hover:border-blue-500/30"
        >
          <div class="absolute right-0 top-0 p-4">
            <button
              :id="`target-delete-button-${target.name}`"
              type="button"
              class="rounded-lg p-2 text-slate-500 transition-colors hover:text-red-400"
              title="删除整个目标配置"
          ...
          <h4 class="mb-2 flex items-center gap-2 text-xl font-bold">
            ...
            <span class="text-slate-100">{{ target.name }}</span>
          </h4>
          <div class="mb-6 mt-2 flex items-center gap-4 border-b border-slate-800/50 pb-4 text-sm text-slate-400">
          ...
          <div>
            <h5 class="mb-3 flex items-center gap-2 text-sm font-medium text-slate-400">
            ...
            <div v-else-if="targetSkillsData[target.name]?.error" class="rounded-xl border border-red-500/20 bg-red-500/5 py-6 text-center">
              <div class="mb-2 text-sm text-red-400">
                获取失败: {{ targetSkillsData[target.name].error }}
              </div>
            ...
            <div v-else-if="!targetSkillsData[target.name]?.skills?.length" class="rounded-xl border border-dashed border-slate-800 bg-slate-800/20 py-6 text-center text-sm text-slate-500">
              暂无上传的技能
            </div>
            <div v-else class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
              <div
                ...
                :key="skill.name"
                class="group/skill flex items-start justify-between rounded-xl border border-slate-700/50 bg-slate-800/40 p-4 transition-colors hover:border-slate-600"
              >
                <div class="min-w-0 flex-1">
                  <div class="flex items-center gap-2 truncate font-bold text-slate-200">
                    {{ skill.meta?.name || skill.name }}
                    <span v-if="skill.meta?.version" class="rounded bg-slate-700 px-1.5 py-0.5 font-mono text-[10px] text-slate-300">{{ skill.meta.version }}</span>
                  </div>
                  <div class="mt-1 truncate text-xs text-slate-400" :title="skill.meta?.description">
                ...
                <button
                  ...
                  type="button"
                  class="ml-2 shrink-0 rounded p-1.5 text-slate-500 opacity-0 transition-all hover:bg-red-500/10 hover:text-red-400 group-hover/skill:opacity-100"
                  title="删除该技能"
